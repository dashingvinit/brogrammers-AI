...
const extractUnit = async (splitDocs) => {
  ...
  const extractionPrompt = (documentChunk) => [
    new SystemMessage(`
          ...
          - Try grouping subtopics
          - If a topic is not related to the course, ignore it
          - keep it simple and concise
          - Generate a summary of the entire chunk.(if applicable)
          - Give a title to the summary. (if applicable)
          ...
          - Provide a structured JSON response with the following schema:
            
              {
                [
                  ...
                  "summary": { title: { type: String }, content: { type: String } }
              }
  ...
  const consolidationPrompt = (extractedUnits) => [
    new SystemMessage(`
      ...
      - Limit the total number of units to a maximum of 8-10
 ...
 units:[
          ...
          topics: ["Unique Topic 1", "Unique Topic 2", ...]
        }
      ]       
